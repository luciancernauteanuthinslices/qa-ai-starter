...
async function toFeatureAI(inputFile: string, raw: string, expectedTitle: string): Promise<string> {
...
- The very first line MUST be exactly: "Feature: ${expectedTitle}" (case sensitive).
- Start with "Feature: …".
...
async function processFile(inputFile: string, outPath: string, useAI: boolean) {
  ...
  const storyObj = parseStory(raw, inputFile);
  const expectedTitle = storyObj.feature || storyObj.title || storyObj.id || path.basename(inputFile, path.extname(inputFile));
  let featureText: string;

  if (useAI) {
    const aiText = await toFeatureAI(inputFile, raw, expectedTitle);
    
    // Validate that AI respected the expected title; fallback if not.
    if (aiText.toLowerCase().startsWith(`feature: ${expectedTitle.toLowerCase()}`)) {
      featureText = aiText;
    } else {
      console.warn(`[ai] Title mismatch for ${path.basename(inputFile)}. Expected: "${expectedTitle}", Got: "${aiText.split('\n')[0]}". Falling back to deterministic generation.`);
      featureText = toFeatureFallback(storyObj);
    }
  } else {
    featureText = toFeatureFallback(storyObj);
  }
